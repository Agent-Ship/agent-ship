...
![System context (C4 level 1). Practitioners author an `AgentSpec`; end users speak to or call a compiled harness. `AgentShip` mediates between the caller and the external components that make up the harness. Solid borders mark components that ship today; the dashed component is specified but not yet implemented. \label{fig:c4-context}](figures/architecture.png)

At the container level, `AgentShip` is a monorepo of seven installable packages. `agentship-core` is the vendor-neutral kernel: `AgentSpec`, `RunContext`, the `Engine` abstract base class and its `EngineCapabilities` record, a vendor-neutral `Tool` type, a middleware seam, and the conformance grid. `agentship-langgraph` is the reference engine that compiles an `AgentSpec` into a LangGraph `StateGraph`. `agentship-service` is the FastAPI [@fastapi] application that exposes each agent over `/v1`. `agentship-voice` adds the cascaded speech loop — voice activity detection, speech recognition, the same agent, speech synthesis — behind one three-method adapter seam implemented for both Pipecat and LiveKit. `agentship-observability` is the OpenTelemetry pipeline behind the kernel's `Observer` seam. `agentship-cli` provides `agentship run`, `agentship serve`, `agentship doctor`, and `agentship verify`. A meta-package, `agentship-sdk`, installs the batteries-included stack.

![Container view (C4 level 2). Seven installable packages compose the framework. Entry points sit above adapters, which implement seams defined by the vendor-neutral kernel below; only the reference engine imports LangGraph, and only `agentship-voice` imports a speech framework. \label{fig:c4-container}](figures/capability-grid.png)
...
Adding a `voice:` block makes the same harness answer a microphone. `agentship-voice` assembles a cascaded loop — voice activity detection, speech recognition, the identical `RunnableAgent`, speech synthesis — rather than delegating the turn to a speech-to-speech model, so tools, memory, tenancy and tracing all survive the change of channel. Speech recognisers and synthesisers are named in the spec rather than imported (eight of each are registered, including Deepgram, AssemblyAI, ElevenLabs and Cartesia), and both `Pipecat` [@pipecat] and `LiveKit Agents` [@livekit] implement a three-method adapter seam whose shared contract is one sentence: given what the human said, stream back what the agent says. The human may interrupt mid-utterance, and a witness processor placed downstream of synthesis records what was *spoken* rather than what was generated, so the conversation history matches what the caller actually heard. Each turn returns a per-stage latency trace; the shipped conformance cell asserts first audio inside the design's 850 ms budget, and measures 389 ms against a stated provider profile.
...
![Dynamic view (C4 level 3). One HTTP request flows through authentication and tenant scoping, into the reference engine's supervisor graph, out to a specialist and its tools, and emits a single OpenTelemetry trace. The conversation is loaded and checkpointed per node, so the same turn is resumable after a pause or a crash. \label{fig:c4-dynamic}](figures/turn-lifecycle.png)
