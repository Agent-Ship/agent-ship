"""Offline proof of the chat UI's plumbing — the machinery ``demos/service_client.py`` owns.
...
import demos.service_client as service_client  # noqa: E402
...
def test_the_picker_is_the_services_catalog_and_carries_the_api_key(service):
    ...
    names, error = service_client.fetch_agent_names()
...
def test_resume_value_maps_yes_no_and_passes_other_text_through():
    ...
    assert service_client.resume_value_from_reply("yes") == {"approved": True}
    assert service_client.resume_value_from_reply("no") == {"approved": False}
    assert service_client.resume_value_from_reply("use 3 sources") == "use 3 sources"
...
async def test_a_streamed_turn_renders_frames_and_traces_their_types_and_seq(service):
    ...
    state = service_client.new_state()

    history, box, state, trace = await drain(
        service_client.respond("hi", [], "assistant", True, state)
    )
...
async def test_one_session_id_is_kept_across_turns_and_reminted_on_agent_switch(service):
    ...
    history, _, state, _ = await drain(service_client.respond("hi", [], "assistant", False, state))
    first = state["session_id"]
    assert first, "a conversation thread should be opened on the first turn"
    assert history[-1]["content"] == "hi"

    history, _, state, _ = await drain(service_client.respond("again", history, "assistant", False, state))
    ...
    _, _, state, _ = await drain(service_client.respond("hello", history, "note-taker", False, state))
    assert state["session_id"] != first
...
async def test_a_paused_turn_is_held_then_resumed_on_the_same_session(service):
    ...
    history, _, state, _ = await drain(
        service_client.respond("save a note: buy milk", [], "note-taker", False, state)
    )
    ...
    history, _, state, trace = await drain(
        service_client.respond("yes", history, "note-taker", False, state)
    )
...
async def test_a_failed_turn_is_shown_in_the_chat_not_raised(service):
    ...
    history, box, state, _ = await drain(service_client.respond("hi", [], "assistant", False, state))
