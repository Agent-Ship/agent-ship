...
## Studio — one browser tab for every agent here

```bash
make ui     # starts the service if it is not already up, then opens Studio
```

Studio asks for an API key on first load. In this demo it is **`dev`**.

<p align="center">
  <img src="docs/assets/studio.png" alt="AgentShip Studio showing the demo's nine agents, each with its model and tools, and the trace, latency and spec panels" width="100%">
</p>

Every agent in this repo appears in the left-hand list with what it actually declares — its
model, the tools it may call by name, whether it checkpoints, whether it has a team. Pick one,
send a message, and watch the reply stream while the right-hand panels fill in the trace ids,
the per-stage latency and the spec the agent was built from.

> **Voice is off by default.** The image ships the voice package, but a `voice:` spec has its
> provider keys checked *before the socket binds*, so an unset `DEEPGRAM_API_KEY` would stop
> the whole service rather than that one agent. See [`agents/voice/README.md`](agents/voice/README.md)
> to turn it on.
...
> Slice 8's tests run offline either way.
...
make ui                                     # starts the service if needed, opens Studio
```
...
AgentShip **Studio** is the one interactive front door, and it drives the
...
the service-contract tests, which run with or without a key; the live
